...
export function buildLogoSvg(topColor, botColor) {
  ...
  // MD16: 75% opacity on right half when both colors match.
  const sameColor = !!(topColor && botColor) && topColor.toLowerCase() === botColor.toLowerCase();
  const botOpacity = sameColor ? ' fill-opacity="0.75"' : '';
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 234.6">`
